...
import { Eye, GitBranch, Star, AlertCircle, Package, Zap, Shield, Code, Terminal, ChevronRight, ExternalLink, Download, Chrome, Github, Lock, FileText, ArrowLeft } from "lucide-react";
...
{features.map((f) => (
<div key={f.title} className="group p-6 bg-slate-900/40 border border-sky-400/25 rounded-xl hover:border-sky-400/50 hover:bg-slate-800/50 transition-all duration-300 backdrop-blur-sm">
...
{steps.map((s) => (
<div key={s.num} className="group flex gap-6 p-6 rounded-xl border border-sky-400/25 bg-slate-900/40 hover:bg-slate-800/60 hover:border-sky-400/45 transition-all duration-300 backdrop-blur-sm">
...
<code>
{`# Clone the repo
git clone [https://github.com/user/repo-visualizer.git](https://github.com/user/repo-visualizer.git)
cd repo-visualizer
...
{techStack.map((t) => (
<div key={t.name} className="group p-4 bg-slate-900/50 border border-sky-400/25 rounded-xl text-center hover:border-sky-400/50 hover:bg-slate-800/80 transition-all duration-300 backdrop-blur-sm cursor-default">
...
<button onClick={onBack} className="flex items-center gap-2 text-sky-400 hover:text-sky-300 mb-12 transition-colors font-mono text-sm">
...
<p>Repo Visualizer is built with a "Privacy First" philosophy. We do not collect, store, or transmit any personal data, browsing history, or repository information to external servers. All data processing occurs locally within your browser.</p>
</section>
<section>
<h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
<Zap className="w-5 h-5 text-teal-400" /> 2. API Usage
</h2>
<p>The extension interacts directly with the official <span className="text-sky-400">GitHub REST API</span> to fetch public repository statistics and file contents (such as <code>package.json</code>). These requests are made from your client and are subject to GitHub's own privacy policy and rate limits.</p>
</section>
<section>
<h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
<Eye className="w-5 h-5 text-teal-400" /> 3. No Tracking
</h2>
<p>We do not use cookies, analytics scripts (like Google Analytics), or any form of fingerprinting. The extension does not track which repositories you visit or how long you spend on them.</p>
</section>
<section>
<h2 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
<Code className="w-5 h-5 text-teal-400" /> 4. Open Source Transparency
</h2>
<p>As an open-source project, our entire codebase is available for audit. You can verify exactly how your data is handled by reviewing our source code on GitHub. We encourage community contributions and security audits.</p>
</section>
<div className="p-6 rounded-xl border border-sky-400/20 bg-slate-900/40 mt-12">
<p className="text-sm font-mono italic text-slate-400">Last updated: March 02, 2026. For any questions, please open an issue on the GitHub repository.</p>
</div>
</div>
</div>
</div>
...
<button onClick={onPrivacyClick} className="text-xs font-mono text-slate-400 hover:text-sky-400 transition-colors flex items-center gap-1.5">
<FileText className="w-3.5 h-3.5" /> Privacy Policy
</button>
<a href="[https://github.com/calvintakasi](https://github.com/calvintakasi)" target="_blank" rel="noopener noreferrer" className="text-slate-400 hover:text-sky-400 transition-colors duration-300">
<Github className="w-5 h-5" />
</a>
<span className="text-xs text-slate-400 font-mono">Developed By <a href="[https://www.github.com/calvintakasi](https://www.github.com/calvintakasi)" target="_blank" rel="noopener noreferrer" className="underline underline-offset-4 hover:text-sky-400">calvintakasi</a></span>
</div>
</div>
</footer>
);
...
const handlePrivacy = () => { setView("privacy"); window.scrollTo(0, 0); };
const handleHome = () => { setView("home"); window.scrollTo(0, 0); };
